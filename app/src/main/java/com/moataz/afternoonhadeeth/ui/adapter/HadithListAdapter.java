...
public class HadithListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    static class HadithViewHolder extends RecyclerView.ViewHolder {
        ListHadithDataBinding listHadithBinding;

        HadithViewHolder(@NonNull ListHadithDataBinding itemView) {
        ...
        void setOnClick(Hadith hadith) {
            ...
            listHadithBinding.shareButtonOnClick.setOnClickListener(view -> {
                Intents.INSTANCE.copyText(Objects.requireNonNull(hadith.getHadith()), itemView.getContext(),"");
                Intents.INSTANCE.sharedText(itemView.getContext(), Objects.requireNonNull(hadith.getHadith()), "تم الإرسال من تطبيق حديث الغروب: سيرة النبي ﷺ","");
            });
        }
    }
}